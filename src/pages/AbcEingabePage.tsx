...
export default function AbcEingabePage() {
  const [records, setRecords] = useState<AbcEingabe[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<AbcEingabe | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<AbcEingabe | null>(null);
  const [viewingRecord, setViewingRecord] = useState<AbcEingabe | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getAbcEingabe());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: AbcEingabe['fields']) {
    await LivingAppsService.createAbcEingabeEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: AbcEingabe['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateAbcEingabeEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  return (
    <PageShell
      title="ABC Eingabe"
      subtitle={`${records.length} ABC Eingabe im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="ABC Eingabe suchen..."
          value={search}
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine ABC Eingabe. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
