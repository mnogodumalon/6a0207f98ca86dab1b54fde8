...
import { AbcEingabeViewDialog } from '@/components/dialogs/AbcEingabeViewDialog';
...
const ENTITY_TABS = [
  { key: 'abc_eingabe', label: 'ABC Eingabe', pascal: 'AbcEingabe' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('abc_eingabe');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'abc_eingabe': return (data as any).abcEingabe as AbcEingabe[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'abc_eingabe': return ABCEINGABE_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'abc_eingabe': return {
        create: (fields: any) => LivingAppsService.createAbcEingabeEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateAbcEingabeEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteAbcEingabeEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'abc_eingabe' || dialogState?.entity === 'abc_eingabe') && (
        <AbcEingabeDialog
          open={createEntity === 'abc_eingabe' || dialogState?.entity === 'abc_eingabe'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'abc_eingabe' ? handleUpdate : (fields: any) => handleCreate('abc_eingabe', fields)}
          defaultValues={dialogState?.entity === 'abc_eingabe' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['AbcEingabe']}
          enablePhotoLocation={AI_PHOTO_LOCATION['AbcEingabe']}
        />
      )}
      {viewState?.entity === 'abc_eingabe' && (
        <AbcEingabeViewDialog
          open={viewState?.entity === 'abc_eingabe'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'abc_eingabe', record: r }); }}
        />
      )}
