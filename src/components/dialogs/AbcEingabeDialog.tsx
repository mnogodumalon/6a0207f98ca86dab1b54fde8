...
interface AbcEingabeDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: AbcEingabe['fields']) => Promise<void>;
  defaultValues?: AbcEingabe['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function AbcEingabeDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: AbcEingabeDialogProps) {
  const [fields, setFields] = useState<Partial<AbcEingabe['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'abc_eingabe');
      await onSubmit(clean as AbcEingabe['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<AbcEingabe['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'ABC Eingabe bearbeiten' : 'ABC Eingabe hinzufügen';
