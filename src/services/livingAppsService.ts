...
import type { AbcEingabe, CreateAbcEingabe } from '@/types/app';
...
export class LivingAppsService {
  // --- ABC_EINGABE ---
  static async getAbcEingabe(): Promise<AbcEingabe[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.ABC_EINGABE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as AbcEingabe[];
    return enrichLookupFields(records, 'abc_eingabe');
  }
  static async getAbcEingabeEntry(id: string): Promise<AbcEingabe | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.ABC_EINGABE}/records/${id}`);
    const record = { record_id: data.id, ...data } as AbcEingabe;
    return enrichLookupFields([record], 'abc_eingabe')[0];
  }
  static async createAbcEingabeEntry(fields: CreateAbcEingabe) {
    return callApi('POST', `/apps/${APP_IDS.ABC_EINGABE}/records`, { fields: cleanFieldsForApi(fields as any, 'abc_eingabe') });
  }
  static async updateAbcEingabeEntry(id: string, fields: Partial<CreateAbcEingabe>) {
    return callApi('PATCH', `/apps/${APP_IDS.ABC_EINGABE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'abc_eingabe') });
  }
  static async deleteAbcEingabeEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.ABC_EINGABE}/records/${id}`);
  }
